...
- [3.5.7-r0](https://hub.docker.com/layers/sig9/alpine-openssl/3.5.7-r0)
...
```sh
docker run -v "$(pwd)":/root -it -d --name alpine-openssl sig9/alpine-openssl:3.5.7-r0
```
...
```sh
docker buildx build \
  --build-arg ALPINE=3.24 \
  --build-arg REPOSITORY=3.24 \
  --build-arg OPENSSL=3.5.7-r0 \
  --platform linux/amd64,linux/arm/v6 \
  --output=type=registry \
  --tag sig9/alpine-openssl:3.5.7-r0 \
...
```sh
docker run -it -d --name alpine alpine:3.24.1
